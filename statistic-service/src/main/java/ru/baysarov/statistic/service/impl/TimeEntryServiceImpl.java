...
/**
 * Реализация сервиса для работы с записями о затраченном времени на задачу. Обрабатывает операции
 * сохранения записей о времени, связывая их с задачами.
 */
@Service
...
public class TimeEntryServiceImpl implements TimeEntryService {
   ...
   *
   * @param timeEntryDto объект записи о времени, содержащий информацию о задаче, пользователе и
   *                     затраченном времени
  ...
  public void saveTimeEntry(TimeEntryDto timeEntryDto) {
    Task task = taskRepository.findById(timeEntryDto.getTaskId())
        .orElseThrow(
            () -> new TaskNotFoundException(timeEntryDto.getTaskId()));
